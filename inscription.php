...
if (isset($_POST['inscription'])){
    ...
    $mdphash = password_hash($mdp, PASSWORD_DEFAULT); // Pour la sécurité, conseiller par un ami en dev

    if ($pseudolength >= 55 || $pseudolength <= 2) $erreurpseudo = "Votre pseudo doit contenir entre 2 et 55 caractères";
    ...
    if ($mailexistant == 0) {
        if (empty($erreurpseudo) && empty($erreurmdp) && empty($erreurmdpconf) && empty($erreurmail)) {
            ...
            header("Location: connexion.php?inscription=success"); // renvoie sur une page configuré par une fonction GET
            session_destroy(); // Destruction de la session pour éviter le piratage
            exit(); // on quitte le script PHP
        }
    } else {
...
<head>
    ...
    <title> FootKit - Inscription </title>
    ...
    <main>
        <div style="text-align: center;">
            ...
            <form method="POST">
                ...
                <div class="form-outline mb-4">
                    <input type="text" name="pseudo" placeholder="Votre pseudo ..." required="required" autocomplete="on">
